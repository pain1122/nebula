...
type UseDevToolboxMenuParams = {
    history: NavigateFunction;
    isRootAdmin: boolean;
};

export const useDevToolboxMenu = ({ history, isRootAdmin }: UseDevToolboxMenuParams) => {
    //state data
    ...
    useEffect(() => {
        ...
        if (iscurrentState === 'Widgets') {
            history(isRootAdmin ? "/panel/dev/widgets" : "/dashboard");
    ...
    const toDevToolboxLink = (link?: string) => {
        if (!link || link === "/#" || !link.startsWith("/")) {
            return link;
        }

        return `/panel/dev${link}`;
    };

    const prefixDevToolboxLinks = (items: any[]): any[] => {
        return items.map((item: any) => ({
            ...item,
            link: toDevToolboxLink(item.link),
            subItems: item.subItems ? prefixDevToolboxLinks(item.subItems) : item.subItems,
            childItems: item.childItems ? prefixDevToolboxLinks(item.childItems) : item.childItems,
        }));
    };
    ...
    return isRootAdmin
        ? [
            ...
            ...prefixDevToolboxLinks(menuItems.filter((item: any, index: number) => !(index === 0 && item.isHeader))),
        ]
        : [];
};
